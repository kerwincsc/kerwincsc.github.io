...
*list* 进程被异步地运行, 并且它的输入或输出作为一个文件名出现;
此文件名被当成一个参数传递给当前命令, 作为扩展的结果;

如果使用 *>(list)* 形式, 写到文件将给 *list* 提供 *input*;
如果使用 *<(list)* 形式, 作为参数被传递的文件将被读取以获得 *list* 的输出;
注意, 在 '<' 或 '>' 和左括号之间不会出现空格, 否则该结构会被解释成一个重定向.

进程替换在支持命令管道(FIFOs)的或者支持命名文件打开方法 */dev/fd* 的系统中被支持;


可用时, 进程替换同时进行参数和变量展开, 命令替换和算术扩展;


单词分割
------------------------------------------------------------

shell扫描没有出现在双引号内的参数扩展, 命令替换和算术扩展的结果, 以便进行分词;

shell 把 $IFS 变量的每一个字符作为分隔符,
并且使用这些字符作为域分隔符把其它扩展的结果分隔成单词;
如果 **IFS** 未设, 或者它的值恰恰是 <space><tab><newline>,
则忽略<space>, <tab>和 <newline> 的顺序
